...
(defun step-program (vm)
  (multiple-value-bind (ins operand1 operand2 operand3)
    ...
    (ecase (<instruction>-name ins)
      ...
      (:ifzero (when (zerop operand1)
                 (setf (slot-value vm (decode-register 7))
                       (slot-value vm (decode-register operand2))))
               (format t "jump ~s when (zerop ~s)~%" operand2 operand1))
      (:ifeq (when (/= operand1 operand2)
               (setf (slot-value vm (decode-register 7))
                     (slot-value vm (decode-register operand3))))
             (format t "jump ~s when (eql ~s ~s)~%" operand3 operand1 operand2))
      (:ifneq (when (= operand1 operand2)
                (setf (slot-value vm (decode-register 7))
                      (slot-value vm (decode-register operand3))))
              (format t "jump ~s unless (not (eql ~s ~s)~%" operand3 operand1 operand2))
